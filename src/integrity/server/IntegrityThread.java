...
//               applicationDir + instanceReader.getTagValue("structure","softwarehome"));
         ...
         case REQUEST_FILE:
           ...
           if(csc.containsKey(fileName)){
             ...
             pushFile(new File(applicationDir + instanceReader.getTagValue("structure","softwarehome") + checkSumUtil.FILE_SEP + fileToPush));
           }else{
      ...
      System.out.println("ErrorStartingItegrityThread::IntegrityThread::startListener ");
      e.printStackTrace();
      disconnect();
    } //end try-catch
  } //end startListener
...
//####################################################################//
//System.out.println("LOADING THE FILE LISTING HERE");
    ...
    int index = 0;
//####################################################################//
System.out.println("?? " + applicationDir + instanceReader.getTagValue("structure","softwarehome"));
...
//System.out.println(directoryStructure.toString().replaceAll(" ","::"));
      TagReader iReader = new TagReader();
      iReader.parseFile(directoryStructure.toString().replaceAll(" ","::"));
      csc.setFileSet(iReader);
//System.out.println("--< END LOADING THE FILE LISTING HERE >--");
//####################################################################//
  ...
  private void pushFile(File fileName) throws Exception{
    ...
    try{
      ...
      System.out.println(new java.util.Date() + " :: From: " + openSocket.getInetAddress() + " :: Filename: " + fileName + "  fsize: " + available);
      ...
      while( available > 0){
        ...
        oos.flush();
        System.out.println("\tTotal Read/Write Time: " + (new java.util.Date().getTime() - start));
    }finally{
...
System.out.println("Filename: " + fileName + "  fsize: " + fsize);
    long start = new java.util.Date().getTime();
    byte[] bite = new byte[fsize];
    fins.read(bite);
System.out.println("Read Time: " + (new java.util.Date().getTime() - start));
    start = new java.util.Date().getTime();
    oos.writeObject(bite);
    oos.flush();
System.out.println("Network Write Time: " + (new java.util.Date().getTime() - start));
...
*/
  ...
  public void disconnect() {
    try {
      ...
      System.out.println("ErrorDisconnecting::IntegrityThread::disconnect:: ");
      e.printStackTrace();
    }
  }
}
